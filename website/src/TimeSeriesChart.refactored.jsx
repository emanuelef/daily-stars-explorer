...
// Binning options for time series
...
// Convert a date to the dd-mm-yyyy format needed for chart markers
const formatDateForMarker = (dateObject) => {
...
// Filter results for Hacker News feed
...
const createDefaultChartProps = () => ({
  ...
  dataSource: {
    ...
    // Initialize with empty data to prevent blank screen issues
    data: { _data: [] }
  },
  events: {
    selectionChange: function(ev, scope) {
      if (ev && ev.data) {
        scope.setSelectedTimeRange({
          start: ev.data.start,
          end: ev.data.end,
        });
        scope.handleZoom(ev.data.start, ev.data.end);
      }
    },
    rendered: function(e, scope) {
      setTimeout(() => {
        e.sender.setTimeSelection(scope.selectedTimeRange);
      }, 1000);
    },
    timeMarkerClick: function(eventObj, dataObj, scope) {
      if (dataObj && dataObj["startText"] && scope.currentHNnews.current[dataObj["startText"]]) {
        window.open(scope.currentHNnews.current[dataObj["startText"]]["HNURL"], "_blank");
      }
    }
  }
});
...
function TimeSeriesChart() {
  // Get router parameters and context
  ...
  const defaultRepo = user && repository ? `${user}/${repository}` : "helm/helm";
  ...
  // Chart data and transformation states
  const [ds, setds] = useState(() => {
    const defaultProps = createDefaultChartProps();
    // Make the event handlers work with state
    defaultProps.events.selectionChange = function(ev) {
      if (ev && ev.data) {
    ...
    defaultProps.events.rendered = function(e) {
      setTimeout(() => {
        e.sender.setTimeSelection(selectedTimeRange);
      }, 1000);
    };
    defaultProps.events.timeMarkerClick = function(eventObj, dataObj) {
      if (dataObj && dataObj["startText"] && currentHNnews.current[dataObj["startText"]]) {
  ...
  const [transformation, setTransformation] = useState(queryParams.get("transformation") || "none");
  ...
  // Handle zoom selection in the chart
  const handleZoom = (start, end) => {
    if (ds && ds.dataSource && ds.dataSource.data && ds.dataSource.data._data) {
      ...
      const totalStarsAllTime = ds.dataSource.data._data[ds.dataSource.data._data.length - 1][2];
      
      setZoomedStars(totalStarsSelection);
      setZoomedStarsPercentageTotal(
        ((totalStarsSelection / totalStarsAllTime) * 100).toFixed(2)
      );
    }
  };
  ...
  // Generic function to process feed items and create a date-mapped object
  const processFeedItems = (items, dateField, urlField) => {
    const dateMap = {};
    
    items.forEach(item => {
      const date = new Date(item[dateField]);
      const formattedDate = formatDateForMarker(date);
  ...
  // Generic function to create chart markers from feed items
  const createFeedMarkers = (items, dateField, getLabel, maxItems = 40, markerColor = "#FF6600") => {
  ...
  // Fetch HackerNews feed
  const fetchHNFeed = async (options) => {
    try {
      const repoParsedTmp = parseGitHubRepoURL(selectedRepo);
      let repoName = repoParsedTmp.split("/")[1];

      // Fetch HN posts with both repo name and full path
      ...
      // Update the reference for click handling
      currentHNnews.current = mapHN;

      // Create markers for the chart
      const news = createFeedMarkers(
        hackernews, 
        'CreatedAt', 
        item => `${item.Title}<br>Points: ${item.Points}<br>Comments: ${item.NumComments}`
      );
      ...
      toast.error("Failed to fetch Hacker News data", {
        position: toast.POSITION.BOTTOM_CENTER,
      });
      // Ensure we still show peaks even if feed fails
  ...
  const fetchRedditFeed = async (options) => {
    try {
      ...
      const repoName = parseGitHubRepoURL(selectedRepo).split("/")[1];
      
      // Fetch Reddit posts
      const redditPosts = await fetchReddit(repoName);
      
      // Process Reddit items and update reference for click handling
      currentHNnews.current = processFeedItems(redditPosts, 'created', 'url');

      // Create markers for chart
      const reddit = createFeedMarkers(
        redditPosts, 
        'created', 
        item => `${item.title}<br>Ups: ${item.ups}<br>Comments: ${item.num_comments}`
      );
      ...
      toast.error("Failed to fetch Reddit data", {
        position: toast.POSITION.BOTTOM_CENTER,
      });
      // Ensure we still show peaks even if feed fails
      options.dataSource.xAxis.timemarker = [...currentPeaks.current];
  ...
  const fetchYoutubeFeed = async (options) => {
    try {
      ...
      // Fetch YouTube posts
      let ytPosts = await fetchYT(repoName);
      
      // Filter to only include videos with the repo name in the title
      ytPosts = ytPosts.filter(item => 
        item.title.toLowerCase().includes(repoName.toLowerCase())
      );
      
      // Process YouTube items and update reference for click handling
      currentHNnews.current = processFeedItems(ytPosts, 'published_at', 'video_url');

      // Create markers for chart (showing more YouTube videos since they're typically shorter)
      const youtube = createFeedMarkers(
        ytPosts, 
        'published_at', 
        item => `${item.title}<br>Views: ${formatNumber(item.view_count)}`,
        100 // Show more YouTube videos
      );
  ...
  const updateGraph = async (starHistory, currentTotalStars = 0) => {
    // Remove today's entry if present (to avoid partial day data)
    if (starHistory.length > 1) {
      ...
      // Check if we need to show the "Update" option
  ...
  const fetchAllStars = async (repo, ignoreForceRefetch = false, currentTotalStars = 0) => {
    ...
    if (!status) {
      toast.error("Failed to get repository status", {
        position: toast.POSITION.BOTTOM_CENTER,
      });
      setLoading(false);
      return;
    ...
    fetch(fetchUrl)
      ...
      .then((data) => {
        ...
        if (status.cached && !hasYesterday) {
          // Find the last date in the cached history and calculate missing days
        ...
        // Process peak data for visualization
  ...
  const startSSEUpates = (repo, callsNeeded, onGoing) => {
    console.log("Starting SSE updates for", repo, "needed calls:", callsNeeded, "ongoing:", onGoing);
    ...
    sse.onopen = () => {
      console.log("SSE connection opened for", repo);
    };
    ...
    sse.addEventListener("current-value", (event) => {
      ...
      if (currentValue === callsNeeded) {
        console.log("SSE process complete. Closing connection.");
        closeSSE();
        
        // Give a small delay before fetching to ensure server has processed everything
        setTimeout(() => {
